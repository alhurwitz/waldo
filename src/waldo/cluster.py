...
class FaceCluster:
    ...
    similarity computation (``centroid_matrix @ embedding`` in one call).
    """
    ...
    def assign(
        ...
        self._size += 1
        if source_id is not None and bbox is not None:
            self._sources[self._size - 1].append((source_id, bbox))
        return self._size - 1

    def get_sources(self) -> list[list[tuple[str, tuple[float, float, float, float]]]]:
        """Return the source tracking list for all clusters."""
